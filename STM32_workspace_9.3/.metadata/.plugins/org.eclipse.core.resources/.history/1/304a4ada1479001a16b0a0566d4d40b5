...
void add_element(struct Queue *queue, int val_number){
	...
	if(queue->head != NULL){ // La pile a au moins 1 élément
		struct Element *actual_tail = queue->tail; // creation d'une structure 'actual_element' pour avec les donnees du tail (dernier element)
		actual_tail->next = new_element; // je mets le pointeur de l'element actuel vers le nouvel element
		queue->tail = new_element; // le nouvel element devient le tail de la pile
	}
...
int pop_element(struct Queue *queue){
    ...
    if (queue->head != NULL){  //  verification qu'il y ai un element à enlever
        struct Element *element_to_remove = queue->head; // mettre la tete de la pile dans element_to_remove
...
void print_queue(struct Queue *queue){
	...
	struct Element *element_to_print = queue->head;
	printf("J'affiche ma liste dans l'ordre\n");
	while (element_to_print != NULL){ // tant que j'ai un element à afficher
		printf("%d\n", element_to_print->number); // j'affiche le contenu de mon element
